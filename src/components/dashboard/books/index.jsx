...
const BookForm = ({toggleModal, setIsModal}) => {
    ...
    const bookDesSubmit = async (e) => {
        ...
        setIsLoading(true)
        const {status, data: {message}}  = await BookService.postBook(book)
        setIsLoading(false)
        if(status == 201){
          setIsModal(false)
  ...
  return (
    <>
        <Tab {...{activeTab, tabName: "step-1"}}>
            <ImageForm {...{setFirebaseImageURL, setBookName, handleActiveTab, toggleModal}}/>
        </Tab>
        <Tab {...{activeTab, tabName: "step-2"}}>
            <BookDesForm {...{bookDesSubmit, setDescription, setAuthors, setPublishers, setCategory, setEdition, setBookCategories, toggleModal, isLoading}}/>
        </Tab>
    </>
  )
}
